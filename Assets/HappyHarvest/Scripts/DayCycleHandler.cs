...
namespace HappyHarvest
{
    ...
    /// etc..)
    /// </summary>
    // CHANGED: Set to -10 so this initializes BEFORE ShadowInstances (which default to 0)
    ...
    public class DayCycleHandler : MonoBehaviour
    {
        ...
        public static void RegisterShadow(ShadowInstance shadow)
        {
            ...
            {
#endif
                // CHANGED: Added safety checks to prevent NullReferenceException
                if (GameManager.Instance != null)
                {
                    // Fallback: If handler is null, try to find it (covers edge cases)
                    if (GameManager.Instance.DayCycleHandler == null)
                    {
                        GameManager.Instance.DayCycleHandler = FindFirstObjectByType<DayCycleHandler>();
                    }
